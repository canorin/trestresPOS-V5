"""Endpoints de consulta de estado SII — polling de DTEs y Libros.
...
from crumbpos.core.sii_client.polling import poll_all, poll_dtes
...
def trigger_polling(tenant: TenantContext = Depends(get_tenant)):
    """Ejecuta polling de estado SII para todos los DTEs y Libros pendientes.

    Consulta el SII por cada DTE/Libro con estado 'pendiente' o 'enviado'
    ...
    try:
        ...
        if dte.track_id:
            try:
                ...
                if estado_sii_code:
                    nuevo_estado = ESTADO_DTE_MAP.get(estado_sii_code, "pendiente")
