"""
Firma todos los XMLs del set de pruebas usando la librería facturacion_electronica.

Proceso:
1. Cargar certificado PFX via la librería
2. Para cada sobre (EnvioDTE/EnvioBOLETA):
   a. Extraer cada DTE, limpiar namespaces heredados, firmar individualmente
   b. Reconstruir el sobre con los DTEs firmados
   c. Firmar el sobre (SetDTE)
...
def cargar_firma():
    ...
    firma.verify = False  # No validar XSD localmente
    return firma
...
def firmar_sobre(xml_path: Path, firma: Firma) -> Path:
    ...
    1. Extrae y firma cada DTE individual
    2. Reconstruye el sobre con DTEs firmados
    3. Firma el SetDTE del sobre
    """
    tree = etree.parse(str(xml_path))
    root = tree.getroot()

    # Detectar tag raíz (EnvioDTE o EnvioBOLETA)
    ...
    signed_dtes = []
    for dte_el in dtes:
        # Encontrar el Documento/Liquidacion/Exportaciones dentro del DTE
        doc_el = None
        doc_tag = None
        for child in dte_el:
            tag = child.tag.split("}")[-1] if "}" in child.tag else child.tag
    ...
    # Construir el sobre completo como string
    if es_boleta:
    ...
    # Firmar el sobre (type='env' incluye xmlns:xsi en SignedInfo)
...
def main():
    print("Cargando certificado digital (facturacion_electronica)...")
    firma = cargar_firma()
