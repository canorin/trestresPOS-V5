...
Genera 20 DTEs con datos representativos de operación real
...
Distribución: T33×13, T34×3, T52×2, T56×1, T61×1 = 20 DTEs
"""
import os
import sys
...
# ==================== FACTURAS ELECTRÓNICAS (T33) x13 ====================

def crear_facturas(caf_manager: CAFManager) -> list[DTE]:
    """Genera 13 facturas electrónicas con productos de panadería."""
    facturas = []
    ...
    facturas.append(DTE(
        ...
        items=[
            ...
            ItemDetalle(nro_linea=3, nombre="Kuchen de nuez porción", cantidad=80, precio_unitario=3500),
            ItemDetalle(nro_linea=4, nombre="Galleta de avena 100g", cantidad=120, precio_unitario=890),
        ],
    ))

    # F3: Con descuento por ítem
    facturas.append(DTE(
        tipo_dte=33, folio=caf_manager.siguiente_folio(33),
        fecha_emision=FECHA_EMISION, emisor=EMISOR, receptor=RECEPTOR,
        items=[
    ...
    facturas.append(DTE(
        ...
        fecha_emision=FECHA_EMISION, emisor=EMISOR, receptor=RECEPTOR,
        items=[
            ItemDetalle(nro_linea=1, nombre="Strudel de manzana porción", cantidad=60, precio_unitario=2800),
        ],
    ))

    # F9: Donas y muffins (para NC)
    facturas.append(DTE(
        tipo_dte=33, folio=caf_manager.siguiente_folio(33),
        fecha_emision=FECHA_EMISION, emisor=EMISOR, receptor=RECEPTOR,
        items=[
            ItemDetalle(nro_linea=1, nombre="Dona glaseada unidad", cantidad=200, precio_unitario=890),
            ItemDetalle(nro_linea=2, nombre="Muffin arándano unidad", cantidad=150, precio_unitario=1200),
        ],
    ))

    # F10: Palmeras y pan de pascua
    facturas.append(DTE(
        tipo_dte=33, folio=caf_manager.siguiente_folio(33),
        fecha_emision=FECHA_EMISION, emisor=EMISOR, receptor=RECEPTOR,
        items=[
            ItemDetalle(nro_linea=1, nombre="Palmera de chocolate unidad", cantidad=80, precio_unitario=1100),
            ItemDetalle(nro_linea=2, nombre="Pan de pascua 1kg", cantidad=20, precio_unitario=7900),
        ],
    ))
    ...
    facturas.append(DTE(
        ...
        fecha_emision=FECHA_EMISION, emisor=EMISOR, receptor=RECEPTOR,
        items=[
            ItemDetalle(nro_linea=1, nombre="Queque mármol entero", cantidad=12, precio_unitario=6800),
            ItemDetalle(nro_linea=2, nombre="Queque zanahoria entero", cantidad=8, precio_unitario=7500),
        ],
    ))

    # F12: Sopaipillas y calzones rotos
    facturas.append(DTE(
        tipo_dte=33, folio=caf_manager.siguiente_folio(33),
        fecha_emision=FECHA_EMISION, emisor=EMISOR, receptor=RECEPTOR,
        items=[
            ItemDetalle(nro_linea=1, nombre="Sopaipilla pasada docena", cantidad=50, precio_unitario=4500),
            ItemDetalle(nro_linea=2, nombre="Calzones rotos docena", cantidad=30, precio_unitario=3800),
        ],
    ))
    ...
    facturas.append(DTE(
        ...
        fecha_emision=FECHA_EMISION, emisor=EMISOR, receptor=RECEPTOR,
        items=[
            ItemDetalle(nro_linea=1, nombre="Rol de canela unidad", cantidad=150, precio_unitario=1450),
        ],
    ))
...
def crear_facturas_exentas(caf_manager: CAFManager) -> list[DTE]:
    ...
    exentas.append(DTE(
        ...
        fecha_emision=FECHA_EMISION, emisor=EMISOR, receptor=RECEPTOR,
        items=[
            ItemDetalle(nro_linea=1, nombre="Diseño packaging caja pastelería", cantidad=1, precio_unitario=180000),
            ItemDetalle(nro_linea=2, nombre="Diseño etiqueta producto", cantidad=5, precio_unitario=35000),
        ],
    ))
    ...
    exentas.append(DTE(
        ...
        fecha_emision=FECHA_EMISION, emisor=EMISOR, receptor=RECEPTOR,
        items=[
            ItemDetalle(nro_linea=1, nombre="Capacitación manipulación alimentos", cantidad=1, precio_unitario=250000),
        ],
    ))
    ...
    exentas.append(DTE(
        ...
        fecha_emision=FECHA_EMISION, emisor=EMISOR, receptor=RECEPTOR,
        items=[
            ItemDetalle(nro_linea=1, nombre="Asesoría desarrollo nuevos productos", cantidad=1, precio_unitario=320000),
        ],
    ))
...
def crear_notas_credito(caf_manager: CAFManager, facturas: list[DTE]) -> list[DTE]:
    """Genera 1 nota de crédito referenciando factura F9."""
    notas = []

    f9 = facturas[8]  # F9: donas y muffins
    notas.append(DTE(
        ...
        referencias=[
            Referencia(
                ...
                folio_ref=str(f9.folio),
                fecha_ref=f9.fecha_emision,
                codigo_ref=3,
                razon_ref="Devolución parcial mercadería",
            ),
        ],
    ))
...
def crear_notas_debito(caf_manager: CAFManager, facturas: list[DTE]) -> list[DTE]:
    """Genera 1 nota de débito referenciando factura F3."""
    notas_db = []

    f3 = facturas[2]  # F3: con descuentos
    notas_db.append(DTE(
        tipo_dte=56, folio=caf_manager.siguiente_folio(56),
        fecha_emision=FECHA_EMISION, emisor=EMISOR, receptor=RECEPTOR,
        items=[
            ItemDetalle(nro_linea=1, nombre="Interés por mora 30 días", cantidad=1, precio_unitario=15000),
        ],
        referencias=[
            Referencia(
                nro_linea=1, tipo_doc_ref="33",
                folio_ref=str(f3.folio),
                fecha_ref=f3.fecha_emision,
                codigo_ref=3,
                razon_ref="Cobro interés por mora en pago",
            ),
        ],
    ))
...
def generar_set_simulacion():
    ...
    output_dir = settings.OUTPUT_DIR / "simulacion"
    output_dir.mkdir(parents=True, exist_ok=True)

    caf_manager = CAFManager(settings.CAF_DIR)

    # Saltar folios quemados/usados
    caf_manager._folio_actual[33] = 39   # F26-38 usados
    caf_manager._folio_actual[34] = 24   # F11-23 usados
    caf_manager._folio_actual[52] = 50   # F41-49 usados, F52 consumido en envío anterior
    caf_manager._folio_actual[56] = 23   # F16-22 usados
    caf_manager._folio_actual[61] = 37   # F16-36 usados/quemados, F37 seguro

    print("=" * 60)
    print("GENERANDO SET DE SIMULACIÓN (v2)")
    print("Receptor: 77051056-2 (mismo emisor)")
    print("=" * 60)
    ...
    output_file = output_dir / "SetSimulacion_v2.xml"
